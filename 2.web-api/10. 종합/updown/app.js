...
function makeNumberIcons() {
    //id=numbers인 div 태그 안에 숫자 아이콘을 배치해야한다
...
function checkAnswer($numbers, $target) {
    ...
    //up,down일 경우에는 min과 max값 변경
    //begin, end값 변경
    //aside태그의 애니메이션 담당할 executeUpDownAnimation 호출
    if (secret === answer) {
    ...
    //판별 후에는 아이콘을 재배치
    clearNumberIcons($numbers); //현재 렌더링 되어있는 아이콘들을 전체삭제
    makeNumberIcons();  // min,max가 변경되어 있기 때문에 그에 맞춰 아이콘 재배치
}//정답을 판별해주는 함수
...
function processCorrect($target) {
    ...
    //축하 박스는 평소에는 숨겨져 있다 이제 드러나도 된다는 신호
...
(function () {
    ...
    //생성죈 아이콘들에게 이벤트를 걸어주기 위해
    //아이콘 생성 후 아이콘들의 부모 요소를 리턴받을 예정이다
    const $numbers = makeNumberIcons();

    //숫자 아이콘을 클릭(선택)했을 때의 이벤트 (부모 요소에 이벤트 설정해서 전파)
